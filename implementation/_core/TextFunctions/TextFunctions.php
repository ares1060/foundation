<?php
     ...
     */
    class TextFunctions extends core\AbstractService implements core\IService {
        ...
        function __construct(){
        	...
        	$this->ini_file = $GLOBALS['to_root'].'_core/TextFunctions/TextFunctions.ini';
...
//        	$text = preg_replace("/\n/Usi","<br />",$text);
        	...
        	if(!isset($this->smilies)){
        		...
        		if(is_array($smilies)){
        			foreach($smilies as $s){
        				...
        				$text = str_replace($s['code'], '<img src="'.$this->settings->smilies_path.$s['img'].'" class="smilie" />', $text);
        			}
        		}
        	} else {
        ...
        public function getDateAgo($date){
        	...
        	if($this->settings->show_date_long_ago && $diff >= 31536000){
        		$year = date('Y', $date);
        		$month = date('m', $date);
        		$day = date('d', $date);
		...
		public function getTimeFromString($string, $preg='') {
			$preg = ($preg == '') ? $this->settings->default_rimg_preg : $preg;
